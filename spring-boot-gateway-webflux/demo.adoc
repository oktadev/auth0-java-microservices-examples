...
https start.spring.io/starter.zip bootVersion==3.1.3 \
  ...
  dependencies==cloud-eureka,cloud-gateway,cloud-resilience4j,okta | tar -xzvf -
----
...
. Add a `docker-compose.yml` file in the root directory to start a PostgreSQL instance.
+
[source,yaml]
----
version: '3.1'
...
----
+
At startup, these properties will be converted to environment variables using https://github.com/paulschwarz/spring-dotenv[spring-dotenv].
...
. If you go to `http://localhost:8080/cool-cars`, you won't see any data and there will be an error in your console.
...
. Copy the expired time to https://www.timestamp-converter.com/[timestamp-converter.com] to see when it expires in your local timezone. Wait 30 seconds and refresh the page. You'll see a request for a new token and an updated `Expires At` timestamp in your terminal.
