...
describe('Blog e2e test', () => {
  ...
  const blogSample = { name: 'Celsius withdrawal', handle: 'South' };
  ...
  describe('new Blog page', () => {
    ...
    it('should create an instance of Blog', () => {
      cy.get(`[data-cy="name"]`).type('Bacon').should('have.value', 'Bacon');

      cy.get(`[data-cy="handle"]`).type('Bridge').should('have.value', 'Bridge');
